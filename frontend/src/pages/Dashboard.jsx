...
export default function Dashboard() {
  ...
  const renderInvestor = () => {
    if (invested.isLoading) return <div className="card p-4"><div className="h-6 w-1/2 mb-2 rounded bg-surface-elevated animate-pulse" /></div>;
    ...
    <div>
      ...
      {credit.data && (
        <div className="card p-4 mb-6 grid grid-cols-3 gap-2 text-center">
          <div><div className="text-xl font-semibold tabular">{credit.data.onTime}</div><div className="text-xs text-text-secondary">On-time</div></div>
          <div><div className="text-xl font-semibold tabular">{credit.data.late}</div><div className="text-xs text-text-secondary">Late</div></div>
          <div><div className="text-xl font-semibold tabular" style={{ color: credit.data.def > 0 ? "var(--color-danger)" : "inherit" }}>{credit.data.def}</div><div className="text-xs text-text-secondary">Defaults</div></div>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-6">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 rounded-md text-sm border ${tab === t.key ? "bg-primary text-background border-primary" : "border-border text-text-secondary hover:text-text-primary"}`}>
            {t.label}
          </button>
        ))}
      </div>

      {isLoading && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{[0, 1, 2].map((i) => <div key={i} className="card p-4"><div className="h-6 w-1/2 mb-2 rounded bg-surface-elevated animate-pulse" /><div className="h-4 w-full rounded bg-surface-elevated animate-pulse" /></div>)}</div>}
